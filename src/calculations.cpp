...
void fill_pixel_bufer(pixel_bufer &window_pixel_bufer, const scene_manager &scene) {
    for (int pixel_x = 0; pixel_x < window_pixel_bufer.get_width(); pixel_x++) {
        for (int pixel_y = 0; pixel_y < window_pixel_bufer.get_height(); pixel_y++) {
            ...
            gm_vector<double, 3> pixel_3d_point = get_pixel_3d_pos(pixel, scene);

            gm_line<double, 3> camera_ray(scene.get_camera_pos(), pixel_3d_point - scene.get_camera_pos());


            int                  intersection_sphere_idx = -1;
            gm_vector<double, 3> sphere_point            = gm_vector<double, 3>::POISON();
            if (!scene.get_closest_sphere_intersection(camera_ray, &sphere_point, &intersection_sphere_idx)) {
                draw_pixel(window_pixel_bufer, pixel, scene.get_outsphere_color());
                continue;
            }
            vis_sphere vsiual_sphere = scene.get_sphere(intersection_sphere_idx);    
            

            gm_vector<double, 3> ambient_intensity = cord_mul(scene.get_ambient_intensity(), vsiual_sphere.color);
        
            gm_vector<double, 3> sphere_normal_vec = sphere_point - vsiual_sphere.shape.get_center();
            gm_vector<double, 3> sphere_to_light_vec = scene.get_light_src_center() - sphere_point;
            ...
            gm_vector<double, 3> specular_intensity = get_specular_intensity(sphere_to_light_vec, sphere_point, vsiual_sphere.shape.get_center(), scene.get_camera_pos(), 
            ...
            gm_vector<double, 3> summary_intensity = 
                ambient_intensity + 
